...
def scan_common_ports(target_domain, timeout=2):
    ...
    for port in COMMON_PORTS:
        # Gửi gói tin TCP SYN (flags="S")
        response = sr1(IP(dst=target_ip)/TCP(dport=port, flags="S"), 
                       timeout=timeout, verbose=0)
        
        # Nếu nhận được phản hồi và có cờ SYN-ACK (0x12)
        if response and response.haslayer(TCP) and response[TCP].flags == 0x12:
            open_ports.append(port)
            # Gửi gói tin RST để đóng kết nối ngay lập tức
...
def main():
    ...
    if open_ports:
        print(f"Open common ports: {open_ports}")
        print(open_ports)
    else:
